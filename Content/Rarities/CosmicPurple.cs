...
{
    public class CosmicPurple : CustomDrawRarity
    {
        ...
        public sealed class CosmicTextSnippet(string text) : TextSnippet
        {
            ...
            Vector2 origin, Vector2 baseScale, float time, bool renderSpecialEffects, DynamicSpriteFont font)
        {
            ...
            for (int i = 0; i < snippets.Length; i++)
            {
                ...
                {
                    snippets[i] = new CosmicTextSnippet(textSnippet.Text);
                }
            }
        ...
        {
            var sparkle = ModContent.Request<Texture2D>(nameof(VanillaModding) + "/" + "Assets/UI/CrystalTextSparkle").Value;
            var fontSize = ChatManager.GetStringSize(font, text, new Vector2(1));
            rand.SetSeed(1);
