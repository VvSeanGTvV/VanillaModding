        ...
        public void FirstStage()
        {
            ...
            if (stg == 0)
            {
                if (timer1 > 30)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient) Projectile.NewProjectile(source, NPC.Center - new Vector2(5, 20).RotatedBy(-0.25f * (Frame - 7)), direction.RotatedBy(-0.25f * (Frame - 7)), ModContent.ProjectileType<RedLaser>(), 37, 8);
                    if (Frame < 8)
                    {
                        int frameSpeed = 2;
                        NPC.frameCounter += 0.5f;
